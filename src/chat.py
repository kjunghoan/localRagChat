...
def build_conversation_context(conversation_history, max_conversation_context=6):
   """
...
def extract_response_from_output(full_response):
   """
...
def generate_response(model, tokenizer, prompt, max_tokens, max_length):
   """
...
def chat_turn(model, tokenizer, conversation_history, user_input, max_tokens, max_length):
   """
...
def chat_loop(model, tokenizer, chat_config):
   """
   ...
   while True:
       ...
       response = chat_turn(model, tokenizer, conversation_history, user_input, max_tokens, max_length)
       print(f"\nChatbot: {response}")
